...
export const processKeydown = (vditor: IVditor, event: KeyboardEvent) => {
    ...
    // 斜体、粗体、内联代码块中换行
    const newlineElement = hasClosestByAttribute(
        startContainer,
        "data-newline",
        "1"
    );
    ...
    // 代码块
    const preRenderElement = hasClosestByClassName(
        startContainer,
    ...
    if (
        ...
        if (
            ...
            (blockElement.previousElementSibling.getAttribute("data-type") ===
                "code-block" ||
                blockElement.previousElementSibling.getAttribute(
                    "data-type"
                ) === "math-block")
        ) {
            const rangeStart = getSelectPosition(
                blockElement,
                vditor.ir.element,
                range
            ).start;
            if (
                rangeStart === 0 ||
                (rangeStart === 1 &&
                    blockElement.innerText.startsWith(Constants.ZWSP))
            ) {
        ...
        if (headingElement) {
            ...
            if (
                getSelectPosition(headingElement, vditor.ir.element).start ===
                    headingLength &&
                headingLength !== 0
            ) {
                range.setStart(
